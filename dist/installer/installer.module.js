"use strict";
/**
 * @module installer
 */
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var child_process_1 = require("child_process");
var fs_1 = require("fs");
var path_1 = require("path");
var process_1 = require("process");
var pkg = require("pkg").exec;
var args = process_1.argv.slice(2);
if (args.includes("-h") || args.includes("--help")) {
    console.log("\n    Options:\n        -h              shows this message\n        -f [file]       accepts a .json with an array of scripts\n        --dangerous     overwrites currently installed scripts on your machine\n        linux           builds for linux x64\n        win             builds for windows x64\n        mac             builds for macos x64\n    ");
    process.exit(0);
}
var target = "node12-linux-x64";
if (args.includes("win"))
    target = "node12-win-x64";
else if (args.includes("mac"))
    target = "node12-macos-x64";
var in_file = "scripts.json";
if (args.includes("-f"))
    in_file = args[args.indexOf("-f") + 1];
if (in_file === undefined) {
    console.error("Used -f but didn't specify a file");
    process_1.exit(2);
}
if (!path_1.isAbsolute(in_file))
    in_file = path_1.join(__dirname, in_file);
if (!fs_1.existsSync(in_file)) {
    console.log("Couldn't find file at path " + in_file);
    process_1.exit(1);
}
var dangerous = args.includes("--dangerous");
var scripts = require(in_file);
var omitted_script_count = 0;
var install = function () { return __awaiter(void 0, void 0, void 0, function () {
    var bin, si, s, src, dest, op;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                bin = "/usr/local/bin";
                si = 0;
                _a.label = 1;
            case 1:
                if (!(si < scripts.length)) return [3 /*break*/, 6];
                s = scripts[si];
                src = "" + s.path;
                dest = bin + "/" + s.name;
                op = "[ INSTALLED ]";
                if (fs_1.existsSync(dest)) {
                    if (!dangerous) {
                        omitted_script_count++;
                        op = "[ OMITTED ]";
                        console.log(dest + " " + op);
                        return [3 /*break*/, 5];
                    }
                    child_process_1.execSync("sudo rm " + dest);
                    op = "[ UPDATED ]";
                }
                if (!(s.extension === "js")) return [3 /*break*/, 3];
                return [4 /*yield*/, pkg([src, "--targets", target])];
            case 2:
                _a.sent();
                child_process_1.execSync("sudo cp " + s.name + ".module " + dest);
                return [3 /*break*/, 4];
            case 3:
                if (s.extension === "sh")
                    child_process_1.execSync("sudo cp " + src + " " + dest);
                _a.label = 4;
            case 4:
                child_process_1.execSync("sudo chmod +x " + dest);
                console.log(dest + " " + op);
                if (s.extension === "js")
                    fs_1.unlinkSync(s.name + ".module");
                _a.label = 5;
            case 5:
                si++;
                return [3 /*break*/, 1];
            case 6: return [2 /*return*/];
        }
    });
}); };
var post_install = function () {
    if (!dangerous && omitted_script_count > 0)
        console.log("You should manually update omitted scripts or use --dangerous flag to update them using this script.");
};
install();
post_install();
